...
def record_latency(worker_name: str, gpu_lane: str, model: str, role: str, data: dict[str, Any]) -> None:
    ...
    LATENCY_HISTORY[worker_name].append({
        "ts": _now(), "gpu_lane": gpu_lane, "model": model, "role": role,
        "total_duration_ns": total_duration, "tok_per_sec": tok_per_sec,
    })
...
def queue_defaults(cfg: dict[str, Any], role: str, req: ExecRequest) -> tuple[int, int]:
    ...
    wait_ms = req.queue_wait_ms if req.queue_wait_ms is not None else int(per_role.get("queue_wait_ms", queue_cfg.get("queue_wait_ms", 1500)))
    poll_ms = req.queue_poll_ms if req.queue_poll_ms is not None else int(per_role.get("queue_poll_ms", queue_cfg.get("queue_poll_ms", 200)))
...
def select_preferred_models(cfg: dict[str, Any], worker_name: str, gpu_lane: str, role: str, requested_model: str | None, allow_fallback: bool) -> list[str]:
...
def increment_active(worker_name: str, gpu_lane: str, model: str) -> None:
    ...
    ACTIVE_MODEL_REQUESTS[worker_name][gpu_lane][model] = ACTIVE_MODEL_REQUESTS[worker_name][gpu_lane].get(model, 0) + 1
...
def lane_admission_allowed(cfg: dict[str, Any], worker_name: str, gpu_lane: str, role: str, model: str, allow_burst: bool) -> tuple[bool, str]:
        ...
        for gpu_lane in lane_names:
            ...
            for model in models:
                ...
                if not lane_ok:
                    ...
                    continue
                scored.append((score_candidate(cfg, worker_name, gpu_lane, req.role, model, use_burst), {
                    "worker": worker_name,
...
async def choose_worker_and_model(cfg: dict[str, Any], req: ExecRequest) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    raise HTTPException(status_code=503, detail={"message": f"no healthy eligible workers with an admissible lane/model for role '{req.role}'", "failures": failures[-50:]})
...
async def call_generate_with_retry(cfg, chosen, req):
    ...
    same_retries = int(retry.get("same_worker_retries", 1))

    last_exc = None

    for attempt in range(same_retries + 1):
        try:
            return await call_generate(chosen["worker_url"], req, chosen["model"])
        except Exception as exc:
            last_exc = exc
            record_failure(chosen["worker"], type(exc).__name__, cfg)

    # fallback: try another worker
    alt_workers = role_priority(cfg, req.role)

    for alt in alt_workers:
        if alt == chosen["worker"]:
            continue

        try:
            alt_url = cfg["workers"][alt]["base_url"]
            return await call_generate(alt_url, req, chosen["model"])
        except Exception as exc:
            record_failure(alt, type(exc).__name__, cfg)
            last_exc = exc
...
app = FastAPI(title="Spot Core Control Plane", version="final-v2")
...
async def quarantine_worker(worker_name: str, seconds: int = 1800, reason: str = "manual_quarantine") -> dict[str, Any]:
    ...
    PENALTY_BOX[worker_name] = {"reason": reason, "until": _now() + max(60, seconds), "ts": _now(), "quarantined": True, "failure_count_window": failure_window_count(worker_name, 3600)}
...
async def exec_route(req: ExecRequest) -> ExecResult:
    ...
    data, response_text, started = {}, "", _now()
    try:
        if req.role == "utility" and any(token in chosen["model"] for token in ["embed", "bge-"]):
            ...
            data = await call_generate_with_retry(cfg, chosen, req)
            response_text = data.get("response", "")
    except Exception as exc:
        record_failure(chosen["worker"], type(exc).__name__, cfg)
        append_decision({"ts": started, "worker": chosen["worker"], "gpu_lane": chosen["gpu_lane"], "model": chosen["model"], "role": req.role, "priority": priority_of_request(req), "burst_mode": chosen["burst_mode"], "status": "error", "error": repr(exc), "failures_seen": failures[-20:], "penalty": current_penalty(chosen["worker"])})
        raise
    finally:
        async with ACTIVE_LOCK:
            decrement_active(chosen["worker"], chosen["gpu_lane"], chosen["model"])
    mark_model_warm(chosen["worker"], chosen["model"])
    record_latency(chosen["worker"], chosen["gpu_lane"], chosen["model"], req.role, data)
    append_jsonl(EXEC_HISTORY_PATH, {"ts": started, "worker": chosen["worker"], "worker_url": chosen["worker_url"], "gpu_lane": chosen["gpu_lane"], "gpu_label": chosen["gpu_label"], "role_requested": req.role, "priority": priority_of_request(req), "model_requested": req.model, "model_used": chosen["model"], "burst_mode": chosen["burst_mode"], "prompt_chars": len(req.prompt), "response_chars": len(response_text), "total_duration": data.get("total_duration"), "load_duration": data.get("load_duration"), "prompt_eval_count": data.get("prompt_eval_count"), "eval_count": data.get("eval_count")})
    append_decision({"ts": started, "worker": chosen["worker"], "gpu_lane": chosen["gpu_lane"], "gpu_label": chosen["gpu_label"], "model": chosen["model"], "role": req.role, "priority": priority_of_request(req), "burst_mode": chosen["burst_mode"], "status": "ok", "failures_seen": failures[-20:], "latency": worker_latency_summary(chosen["worker"])})
    return ExecResult(ok=True, worker=chosen["worker"], worker_url=chosen["worker_url"], gpu_lane=chosen["gpu_lane"], gpu_label=chosen["gpu_label"], role_requested=req.role, model=chosen["model"], response=response_text, raw=data)
