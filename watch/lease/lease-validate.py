...
REQ = {"schema","lease_id","correlation_id","owner","target","action","ttl_sec","payload_sha256","payload_stored_path","authority","spot_core_executor_only","worker_self_apply","execution_allowed","mutation_authority"}
...
def main():
    ...
    for n, line in enumerate(INDEX.read_text().splitlines(), 1):
        ...
        if rec["schema"] != "spot.execution_lease.v1":
            fail(f"line {n} bad schema")
        if rec["lease_id"] in leases:
            fail(f"duplicate lease_id {rec['lease_id']}")
        leases.add(rec["lease_id"])
        if rec["authority"] != "lease_only":
            fail(f"line {n} bad authority")
        if rec["spot_core_executor_only"] is not True or rec["worker_self_apply"] is not False:
