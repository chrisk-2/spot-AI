...
p = subprocess.run(
    ["python3", "watch/executor/execution-receipt-registry-snapshot.py"],
    text=True,
    capture_output=True,
)
...
for section_name in ["receipt_schema", "immutability_rules"]:
...
print("RESULT: PASS")
print("module=immutable_execution_receipt_registry mode=read_only advisory_only=true execution_allowed=false mutation_authority=false executor=spot-core")
