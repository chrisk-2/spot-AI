...
p = subprocess.run(
    ["python3", "watch/executor/replay-safe-token-snapshot.py"],
    text=True,
    capture_output=True,
)
...
for section_name in ["token_schema", "replay_rules"]:
...
print("RESULT: PASS")
print("module=replay_safe_execution_token_model mode=read_only advisory_only=true execution_allowed=false mutation_authority=false executor=spot-core")
