...
jq -r -n \
  ...
  def readiness_strip:
    ...
    "<div class=\"card\"><h2>Control Surface: <span class=\""+(if (($o.health.spot_core // false) and ($o.health.mcp_local // false) and ($o.validation.fresh // false)) then "ok" else "warn" end)+"\">"+(if (($o.health.spot_core // false) and ($o.health.mcp_local // false) and ($o.validation.fresh // false)) then "OK" else "WARN" end)+"</span></h2><p>Spot Core: <b>"+safe($o.health.spot_core)+"</b></p><p>MCP Local: <b>"+safe($o.health.mcp_local)+"</b></p><p>Validation: <b>"+safe($o.validation.status)+"</b> · Age: <b>"+safe($o.validation.age_minutes)+"m</b></p><p class=\"meta\">Uptime: "+safe($o.health.uptime_sec)+"s · Commit: "+safe($o.git.commit)+"</p></div>"+
...
"<p class=\"meta\">Generated: "+safe($s.generated_at)+" · Feeds: <code>spot.json</code> <code>history.json</code> <code>incidents.json</code> <code>acks.json</code> <code>operator-readiness.json</code> <code>meta.json</code></p></body></html>"
'
