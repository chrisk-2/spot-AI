...

{
  ...
  for host in "${HOSTS[@]}"; do
    ...
    if [[ "$ssh_ok" == true ]]; then
      ...
      if [[ "$host" == "spot-core" ]]; then
        dns_raw="$(cat /etc/resolv.conf 2>/dev/null || true)"
      else
        dns_raw="$(ssh_quick "$host" "cat /etc/resolv.conf 2>/dev/null" || true)"
      fi

      dns_list="$(echo "$dns_raw" | awk '/nameserver/ {print $2}' | sort -u | tr '\n' ' ')"

      if [[ -n "$dns_list" ]]; then
        dns_servers_json="$(printf '%s\n' $dns_list | sed '/^$/d' | jq -R . | jq -s .)"
      fi
      ...
      if echo "$dns_raw" | grep -q "127.0.0.53"; then
        ...
        if [[ "$host" == "spot-core" ]]; then
          resolved_status="$(resolvectl status 2>/dev/null || true)"
        else
          resolved_status="$(ssh_quick "$host" "resolvectl status 2>/dev/null || true" || true)"
        fi

        if echo "$resolved_status" | grep -q "DNS Servers: $GOOD_DNS_1 $GOOD_DNS_2"; then
          dns_ok=true
          dns_servers_json="$(printf '%s\n%s\n' "$GOOD_DNS_1" "$GOOD_DNS_2" | jq -R . | jq -s .)"
        else
          dns_ok=false
        fi
        ...
        if [[ "$host" == "dns-core" ]]; then
          dns_ok=true
        else
          dns_ok=false
        fi

        dns_servers_json='["127.0.0.1"]'
    ...
    "$host" "$POLICY_FILE" "$REMEDIATION_STATE_FILE" "$ssh_ok" "$service_ok" "$models_json" "$dns_ok"
import json, sys
...
PY
    ...
    if echo "$alerts_json" | jq -e '.[] | select(. == "dns_bad")' >/dev/null 2>&1; then
      DNS_FIX_QUEUE+=("$host")
    fi
    ...
    echo -n "\"dns\": {\"ok\": ${dns_ok}, \"servers\": ${dns_servers_json}, \"mode\": \"${dns_mode}\"}, "
...
if (( ${#DNS_FIX_QUEUE[@]} > 0 )); then
  for host in "${DNS_FIX_QUEUE[@]}"; do
    echo "[ACTION] dns_bad on $host" | tee -a "$LOG_FILE"
    ~/spot-stack/watch/fix-dns.sh "$host" >> "$LOG_FILE" 2>&1 || true
  done
fi
