...
cmd_create() {
  ...
  local executor_summary backup_binding_summary manifest_contract_summary manifest_dry_run_summary
  local executor_failure_log backup_binding_failure_log manifest_contract_failure_log manifest_dry_run_failure_log
  ...
  python3 - "$out" \
    ...
    "$manifest_dry_run_failure_log" <<'PY'
import json
import sys
from pathlib import Path
...
logs = {
    "executor_preflight_failure_validation": Path(sys.argv[6]).read_text(),
    "backup_binding_contract_failure_validation": Path(sys.argv[7]).read_text(),
    "backup_artifact_manifest_contract_failure_validation": Path(sys.argv[8]).read_text(),
    "backup_artifact_manifest_dry_run_failure_validation": Path(sys.argv[9]).read_text(),
}
