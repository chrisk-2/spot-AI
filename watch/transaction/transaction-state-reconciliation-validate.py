...
p = subprocess.run(
    ["python3", "watch/transaction/transaction-state-reconciliation-snapshot.py"],
    text=True,
    capture_output=True,
)
...
for section_name in ["reconciliation_schema", "reconciliation_rules"]:
...
print("RESULT: PASS")
print("module=transaction_state_reconciliation mode=read_only advisory_only=true execution_allowed=false mutation_authority=false executor=spot-core")
