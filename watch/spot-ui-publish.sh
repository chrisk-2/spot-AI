...
publish_once(){
  need_file "$SPOT_UI_SCRIPT"; need_file "$SPOT_UI_HISTORY_SCRIPT"; need_file "$SPOT_UI_RENDER_HTML"; need_file "$SPOT_UI_INCIDENT_LEDGER"; need_file "$SPOT_UI_ACK"
  mkdir -p "$SPOT_UI_OUT_DIR" "$SPOT_UI_TMP_DIR"
  local ts html_tmp json_tmp meta_tmp hist_tmp inc_tmp ack_tmp
  ts="$(date -u +'%Y-%m-%dT%H:%M:%SZ')"
  html_tmp="${SPOT_UI_TMP_DIR}/index.html.$$"; json_tmp="${SPOT_UI_TMP_DIR}/spot.json.$$"; meta_tmp="${SPOT_UI_TMP_DIR}/meta.json.$$"; hist_tmp="${SPOT_UI_TMP_DIR}/history.json.$$"; inc_tmp="${SPOT_UI_TMP_DIR}/incidents.json.$$"; ack_tmp="${SPOT_UI_TMP_DIR}/acks.json.$$"
  ...
  SPOT_UI_JSON="$json_tmp" SPOT_UI_HISTORY_JSON="$hist_tmp" SPOT_UI_INCIDENTS_JSON="$inc_tmp" SPOT_UI_ACKS_JSON="$ack_tmp" bash "$SPOT_UI_RENDER_HTML" > "$html_tmp"
  printf '{"published_at":"%s","source":"%s"}\n' "$ts" "$SPOT_UI_SCRIPT" > "$meta_tmp"
  mv "$html_tmp" "${SPOT_UI_OUT_DIR}/index.html"; mv "$json_tmp" "${SPOT_UI_OUT_DIR}/spot.json"; mv "$hist_tmp" "${SPOT_UI_OUT_DIR}/history.json"; mv "$inc_tmp" "${SPOT_UI_OUT_DIR}/incidents.json"; mv "$ack_tmp" "${SPOT_UI_OUT_DIR}/acks.json"; mv "$meta_tmp" "${SPOT_UI_OUT_DIR}/meta.json"
